...
class Trainer(BaseTrainer):
    ...
    def _train(self, face_id, epoch, dataset, logger):
        ...
        for sub_idx in range(1, self.sub_epoch + 1):
            ...
            dataset.warp_data(warp=True, to=128)
            merged_dataset.append(dataset)
            ...
            for batch_idx, ((warp64, real64), (warp128, real128)) in enumerate(dataloader):
                warp64, real64 = warp64.to(self.device), real64.to(self.device)
                warp128, real128 = warp128.to(self.device), real128.to(self.device)
                ...
                # DISCRIMINATOR
                self.optD.zero_grad()
                output_pos = self.netD(t.cat([real128, warp128], 1))
                output_neg = self.netD(t.cat([fake_s2.detach(), warp128], 1))
                ...
                # GENERATOR
                self.optG.zero_grad()
                output_neg = self.netD(t.cat([fake_s2, warp128], 1))
                lossG = define_lossG(fake_s2, real128, output_neg, device=self.device)
                lossG.backward()
                self.optG.step()
        ...
        img_list = [warp128, fake_s2, real128]
        imwrite(img_list, fname, size=8)
    ...
    def convert(self, source_id, target_id, dataset, size=8):
        ...
        netG_path = self.get_path('netG', target_id)
        self.netG = get_model(  
            'netG_s2_' + target_id, S2GEN, path=netG_path).to(self.device)
        ...
        for batch_idx, (warped, target) in enumerate(dataloader):
            ...
            output_s1 = self.decoder(self.encoder(warped))
            output_s2 = self.netG(output_s1)

            output_file = os.path.join(this_output_dir, '{}_s1.png'.format(batch_idx))
            img_list = [warped, output_s1]
            imwrite(img_list, output_file, size=64, n=8)
            output_file = os.path.join(this_output_dir, '{}_s2.png'.format(batch_idx))
            img_list = [output_s2]
            imwrite(img_list, output_file, size=128, n=8)