...
class LSGANLoss(nn.Module):
    def forward(self, output, dreal, dfake, device='cuda'):
        ...
        loss_f = nn.MSELoss()
        # LS loss
        loss_G = loss_f(output, t.ones(output.shape).to(device))
        # MSE for real
        loss_real = loss_f(dreal, t.ones(dreal.shape).to(device))
        # MSE for fake
        loss_fake = loss_f(dfake, t.zeros(dfake.shape).to(device))
        return 0.5 * loss_G + loss_real + loss_fake
